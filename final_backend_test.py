#!/usr/bin/env python3
"""
Final Comprehensive Backend API Test Suite for Contact Book Application
Tests all endpoints with clean data for each run
"""
...
class ContactBookAPITester:
    def __init__(self, base_url: str = "http://localhost:8001"):
        ...
        self.test_results = []
        self.test_id = str(uuid.uuid4())[:8]  # Unique ID for this test run
    ...
    def test_create_contact(self):
        ...
        contact_data = {
            "name": f"John Doe {self.test_id}",
            "phones": [{"number": "555-1234", "label": "mobile"}],
            "emails": [{"email": f"john_{self.test_id}@example.com", "label": "personal"}],
            "category": "Friends",
    ...
    def test_create_custom_category(self):
        ...
        category_name = f"Colleagues_{self.test_id}"
        response = self.make_request("POST", f"/api/categories?name={category_name}&color=%23FF6B6B")
        
        if response and response.status_code == 201:
            data = response.json()
            if "category_id" in data and data.get("name") == category_name:
    ...
    def test_duplicate_contact_detection(self):
        """Test 13: Duplicate Contact Detection"""
        if not self.token:
        ...
        duplicate_contact = {
            ...
            "emails": [{"email": f"john.duplicate_{self.test_id}@example.com", "label": "work"}],
            "category": "Work"
        }
            ...
            if response.status_code == 400:
                try:
                    ...
                    if "already exists" in error_msg.lower():
                        ...
                        self.log_test("Duplicate Contact Detection", True, f"Duplicate prevented with message: {error_msg}")
                except:
    ...
    def run_all_tests(self):
        ...
        print(f"🚀 Starting Contact Book API Tests (Test ID: {self.test_id})...")
        print("=" * 60)
